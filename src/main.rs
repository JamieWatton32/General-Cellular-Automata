...
fn main() {
    ...
    let window = video_subsystem.window("rust-sdl2 demo", 824, 480)
        .position_centered()
        .build()
        .unwrap();
    ...
    'running: loop {
        ...
        grid.cells[x as usize][y as usize].x = x;
        grid.cells[x as usize][y as usize].y = y;
        
        for i in 0..grid.rows{
            for j in 0..grid.cols{
                grid.update_grid(i as usize, j as usize);
            }
        }
